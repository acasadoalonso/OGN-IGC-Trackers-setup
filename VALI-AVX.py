...
DDB_URL = "http://ddb.acasado.name:60082/download/?j=2"  				# the OGN DDB source
...
def tstdebug(trksignbin,k):
   ...
   return PublicKey
#######
def gettrkpublickey(usb=0, prt=False):		# Get the public key from the trackera
# -------------------------------------#
    ...
    while cnt < 16:
        ...
        if len(line) == 0:      		# end of data ???
           if eol:
              ...
              eol=True
              continue
        if not found and line[0:4] != b'----':
           continue
        if line[0:10] == b'-----BEGIN':
           found = True
        l=line.decode('utf-8')
        publickey += l
...
if os.name != 'nt':			# just report the version, not valid on NT or bundles from pyinstaller
   ...
   if bundle_dir[0:9] != "/tmp/_MEI":
      print("Program Version:", time.ctime(os.path.getmtime(os.path.abspath(__file__))))
      print("=========================================")
...
if prttxt == 'True':
   ...
   prt=False
strongvaltxt = args.strongvaltxt        # strong validation on|off
if strongvaltxt == 'True':
   strongval=True
else:
   strongval=False
...
if debug:
   if strongval: 
      print (gettrkpublickey(usb).encode('utf-8'))
   print (PublicKey)
if strongval:
   PublicKey=gettrkpublickey(usb).encode('utf-8')
